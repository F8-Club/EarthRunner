...
public class StepCounterActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().hasExtra(AppConstants.EXTRA_CAMERA_SCAN)) {
			String content = getIntent().getExtras().getString(
					AppConstants.EXTRA_CAMERA_SCAN);
		...
		registerReceiver(updateReceiver, new IntentFilter(
				AppConstants.BROADCAST_STEP_UPDATE));
	}
	...
	class StepUpdateReceiver extends BroadcastReceiver {
		...
		public StepUpdateReceiver(Activity activity) {
			...
			fieldStepSpeed = (TextView) activity
					.findViewById(R.id.fieldStepsMin);
		}
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(AppConstants.BROADCAST_STEP_UPDATE)
					&& intent
							.hasExtra(AppConstants.BROADCAST_STEP_UPDATE_EXTRA)) {
				StepCounterUpdate stepUpdate = (StepCounterUpdate) intent
						.getSerializableExtra(AppConstants.BROADCAST_STEP_UPDATE_EXTRA);
				...
				fieldStepSpeed
						.setText(String.valueOf(stepUpdate.getStepSpeed()));
			}
		}
